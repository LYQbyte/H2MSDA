...
# from hyperboloid_lorentz import Lorentz
from files.frechet import frechet_mean


class RiemannianBatchNorm(nn.Module):
    def __init__(self, dim, manifold,args=None):
    ...
    def forward(self, x, training=True, momentum=0.9):
        ...
        # if hasattr(self.args,'currently_training'): ##cole hack so we can communicate back and forth
        #     training=self.args.currently_training
        ...
        if training:
            ...
            print(x,'XXXX')
            print(self.man,"MANFRED")
            print(self.args.frechet_B,"FRECH BBBBBB")
            input_mean = frechet_mean(x, self.man,self.args.frechet_B)
            # skg
            # input_mean = frechet_mean(x, self.man)
            input_var = self.man.frechet_variance(x, input_mean)
